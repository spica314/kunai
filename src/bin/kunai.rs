...
fn main() {
    ...
    if let Some(unify_matches) = matches.subcommand_matches("unify") {
        ...
        let path = std::path::PathBuf::from(&format!("{}/kunai/{}", dirs::cache_dir().unwrap().to_str().unwrap(), testcase_dir));
        let res = kunai::judge::judge(binname, &path, &std::time::Duration::from_secs(2));
        let mut ok = true;
        use termion::{color,style};
        let green_bold = |s: &str| format!("{}{}{}{}{}", style::Bold, color::Fg(color::LightGreen), s, color::Fg(color::Reset), style::Reset);
        let yellow_bold = |s: &str| format!("{}{}{}{}{}", style::Bold, color::Fg(color::LightYellow), s, color::Fg(color::Reset), style::Reset);
        for (r,testname) in &res {
            ...
            match r {
                ...
                JudgeResult::WrongAnswer{ stdout, expected, stderr } => {
                    ok = false;
